...
acf(df$charges)
dwtest(df$charges~1)
# a graph shows auto correlation, we have the blue threshold and because the lines are not in the threshold means there is correlation within the data in the column we check. we can always create new variable with less observations and compare just 100 observation for example.
#BUT the solution for would better be to RANDOM THE ORDER OF THE OBSERVATIONS.
#here we can see the data is not correlated and now we can do the shapiro test - the first line is always one and the rest is in the threshold so it's independent
```
